...
open BigOperators CategoryTheory CategoryTheory.Limits PowerSeries
...
`∑ᵢ μ(Mᵢ) Xⁱ ∈ ℤ⟦X⟧`. -/
def poincareSeries : ℤ⟦X⟧ := PowerSeries.mk fun n ↦ μ (FGModuleCat.of (𝒜 0) (ℳ n))
...
variable {𝒜} in
/-- Suppose `a₀, ..., aₙ` with degree `d₀, ..., dₙ`, we can consider the power series
`∏ᵢ (1 - X^{dᵢ})`, this power series is invertible, because its constant coefficient is one. -/
...
lemma poles_inv_eq' :
  ...
  refine Finset.prod_eq_one fun ⟨i, hi⟩ _ ↦ mul_invOfUnit _ _ ?_
...
/-- Statement of Hilbert-Serre theorem: Let `A` be a notherian graded ring and `M` a finite graded
module over `A` and `μ` an additive function from the category of finitely generated `A₀`-modules.
...
abbrev statement' (s : ℕ) : Prop :=
  ∀ (A M : Type u) [CommRing A] [IsNoetherianRing A] [AddCommGroup M]
  [Module A M] [Module.Finite A M] (𝒜 : ℕ → AddSubgroup A) (ℳ : ℕ → AddSubgroup M)
  [GradedRing 𝒜] [DirectSum.Decomposition ℳ] [SetLike.GradedSMul 𝒜 ℳ]
  (μ : (FGModuleCat (𝒜 0)) ⟹+ ℤ) (S : generatingSetOverBaseRing 𝒜) (_ : S.toFinset.card = s),
...
lemma eventually_eq_zero_of_empty_generatorSet :
  ...
  · refine ⟨1, fun n _ x ↦ ?_⟩
    have eq2 := Submodule.span_empty (R := A) (M := M) ▸ Finset.coe_empty ▸ ne_empty ▸ T.span_eq
  ...
  let maxDeg : ℕ := Finset.image deg Finset.univ |>.max' (by
    ...
    exact Finset.nonempty_iff_ne_empty.2 ne_empty)
  ...
  replace eq0 := calc _
    ...
    _ = Submodule.span (𝒜 0) {x : ℳ n | ∃ ω : M, ∃ (_ : ω ∈ T.toFinset), x = ω } := by
        congr
        ext x
        refine exists_congr fun m ↦ exists_congr fun _ ↦ ⟨?_, fun h ↦ ⟨_, rfl, ?_, h ▸ ?_⟩⟩
  ...
  refine Finset.sum_eq_zero fun x hx ↦ show GradedModule.proj _ n ((f x : A) • (x : M)) = 0 from ?_
...
variable {𝒜} {d : ℕ} (x : A) (deg_x : x ∈ 𝒜 d)

/-- Let `x` be a homogeneous element, then the set of elements annilaited by `x` (i,e `x • m = 0`)
forms a homogeneous submodule. Denote this module as `K`. -/
...
/-- The exact sequence `0 → Kₙ → Mₙ → M_{n + d} → L_{n + d}`. More accurately,
`0 → K_{n - d} → M_{n - d} → Mₙ → Lₙ`. -/
@[simps!]
noncomputable def anExactSeq (i : ℕ) (ineq : d ≤ i) : ComposableArrows (FGModuleCat (𝒜 0)) 5 :=
  .mk₅ (0 : 0 ⟶ FGModuleCat.of _ <| (KER ℳ x deg_x).grading (i - d))
...
lemma anExactSeq_complex (i : ℕ) (ineq : d ≤ i) : (anExactSeq ℳ x deg_x i ineq).IsComplex := by
  ...
  interval_cases j
  · ext m
    simp only [Fin.zero_eta, anExactSeq_map, ComposableArrows.Precomp.map_zero_one', zero_comp]
  · ext m
  ...
  · ext m
    simp only [anExactSeq_obj, ComposableArrows.Precomp.obj_succ, ComposableArrows.precomp_obj,
      ComposableArrows.Precomp.obj_one, ComposableArrows.mk₁_obj, ComposableArrows.Mk₁.obj]
    ext
    erw [QuotientAddGroup.eq_zero_iff]
    exact ⟨m.1, trivial, rfl⟩
  · ext m
    simp only [anExactSeq_map, ComposableArrows.Precomp.map_succ_succ, ComposableArrows.precomp_map,
      ComposableArrows.Precomp.map_one_succ, ComposableArrows.mk₁_map, ComposableArrows.Mk₁.map,
      comp_zero]
...
lemma anExactSeq_exact₀ (i : ℕ) (ineq : d ≤ i) :
  ...
  simp only [FGModuleCat.exact_iff, Nat.reduceAdd, Fin.mk_one, Fin.zero_eta, anExactSeq_map,
    ComposableArrows.Precomp.map_zero_one, ModuleCat.hom_zero, LinearMap.range_zero]
...
lemma anExactSeq_exact₃ (i : ℕ) (ineq : d ≤ i) :
  ...
  rw [FGModuleCat.exact_iff]
  simp only [anExactSeq, ComposableArrows.mk₅, ComposableArrows.mk₄, ComposableArrows.mk₃,
    ComposableArrows.mk₂, ComposableArrows.precomp_map, ComposableArrows.Precomp.map,
    ComposableArrows.mk₁_map, ComposableArrows.Mk₁.map, ModuleCat.hom_zero, LinearMap.ker_zero,
...
lemma key_lemma : ∃ (p : Polynomial ℤ), (1 - PowerSeries.X ^ d) * μ.poincareSeries 𝒜 ℳ =
  ...
  set p : Polynomial ℤ := (μ.poincareSeries 𝒜 ℳ).trunc d -
  ...
  have eq0 (q : ℤ⟦X⟧) : coeff _ i (algebraMap _ ℤ⟦X⟧ (q.trunc d)) =
    ...
    rw [show algebraMap (Polynomial ℤ) ℤ⟦X⟧ = (Polynomial.coeToPowerSeries.algHom (R := ℤ) ℤ) by
      rfl]
    simp only [RingHom.coe_coe, Polynomial.coeToPowerSeries.algHom_apply, Algebra.id.map_eq_id,
      map_id, id_eq, Polynomial.coeff_coe, coeff_trunc]
  have eq1 : ∑ x ∈ Finset.antidiagonal i, (if x.1 = d then μ (.of (𝒜 0) (ℳ x.2)) else 0) =
  ...
  · rw [if_pos ineq, if_pos ineq, if_neg (by linarith), if_neg (by linarith), sub_zero, add_zero]
    ...
    simp only [anExactSeq_obj, ComposableArrows.Precomp.obj_one, Fin.zero_eta,
      ComposableArrows.precomp_obj, ComposableArrows.Precomp.obj_zero,
...
/-- If `A = A₀[S, s]`, define `A'` as `A₀[S]`. Then `A'` has grading defined by `n`-th grading
being `Aₙ ∩ A₀[S]`. -/
...
lemma AZeroToA'Zero_comp_A'ZeroToAZero :
  ...
  ext ⟨⟨x, hx1⟩, hx2⟩
  rw [RingHom.comp_apply, AZeroToA'Zero_apply_coe_coe, RingHom.id_apply, A'ZeroToAZero_apply_coe]
...
def generatingSet' : generatingSetOverBaseRing (𝒜' S x S' hS') where
  toFinset := S'.attach.image fun x : S' ↦ ⟨x, by
    rw [mem_A', Algebra.mem_adjoin_iff]
    exact Subring.subset_closure <| Or.inr x.2⟩
  deg {x} hx := S.deg (by
    ...
    specialize ha (R.map (A' S x S' hS').toSubring.subtype) (by
      ...
      · rintro _ ⟨a, rfl⟩
        let a' : 𝒜' S x S' hS' 0 := ⟨⟨(a : A), by
          ...
          exact Subring.subset_closure <| Or.inl ⟨a, rfl⟩⟩, a.2⟩
        exact ⟨a', hR.1 ⟨a', rfl⟩, rfl⟩
      · intro x hx
...
lemma induction : statement'.{u} (N + 1) := by
  ...
  have eq_poles : S.poles.val = (generatingSet' S s S' hS1').poles.val * (1 - X^d : ℤ⟦X⟧) := by
    ...
    have eq0 := calc ∏ i ∈ S.toFinset.attach, (1 - X ^ S.deg i.2 : ℤ⟦X⟧)
        _ = ∏ i ∈ (insert s S').attach, (1 - X ^ S.deg (hS1' ▸ i.2 : i.1 ∈ S.toFinset) : ℤ⟦X⟧) := by
    ...
    · conv_lhs => rw [← Finset.prod_attach]
      ...
      · refine fun i _ ↦ ⟨⟨(i : A), ?_⟩, ?_⟩
        · rw [mem_A']
          ...
          simp only [Finset.mem_image, Finset.mem_attach, true_and, Subtype.exists] at h
          obtain ⟨a, ha1, ha2⟩ := h
          rw [← Subtype.ext_iff.1 ha2, Algebra.mem_adjoin_iff]
        ...
        · simp only [Finset.mem_image, Finset.mem_attach, Subtype.mk.injEq, true_and,
          ...
          simp only [Finset.mem_image, Finset.mem_attach, true_and, Subtype.exists] at h
          obtain ⟨a, ha1, ha2⟩ := h
          exact Subtype.ext_iff.1 ha2 ▸ ha1
    ...
    · intro r
      ...
      exact hs r
  have eq_pole' : (generatingSet' S s S' hS1').poles⁻¹ = S.poles⁻¹ * (1 - X^d : ℤ⟦X⟧) := by
    symm
