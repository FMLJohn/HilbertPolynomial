...
/-- A finite homogeneous generating set of an ideal `I` is a set of non-zero homogeneous
elements that spans `I`. -/
...
noncomputable def deg {a : A} (h : a ∈ S) : ℕ := S.homogeneous h |>.choose

omit noetherian_ring [GradedRing 𝒜] in
lemma mem_deg {a : A} (h : a ∈ S) : a ∈ 𝒜 (S.deg h) := S.homogeneous h |>.choose_spec
...
noncomputable def Irrelevant :
  ...
  let H := Ideal.fg_iff_homogeneously_fg _  |>.mp <| isNoetherianRing_iff_ideal_fg A |>.mp
    inferInstance (HomogeneousIdeal.irrelevant 𝒜).toIdeal
  { toFinset := H.choose
...
lemma irrelevant.deg_pos {a : A} (h : a ∈ S) : 0 < deg _ h := by
  ...
  rw [rid] at h_deg1
  erw [span_eq, HomogeneousIdeal.mem_irrelevant_iff, GradedRing.proj_apply,
...
lemma irrelevant.adjoin_eq_top :
  ...
  suffices ∀ (n : ℕ), 𝒜 n.succ =
    ...
    refine AddSubgroup.iSup_induction (C := fun y ↦ y ∈ Algebra.adjoin (𝒜 0) (S.toFinset : Set A))
      (fun (s : {s : S.toFinset | S.deg s.2 ≤ m + 1 }) ↦ (s : A) • 𝒜 (m.succ - deg _ s.1.2)) hx
        ?_ ?_ ?_
...
/-- If `f` maps `aᵢ` to `nᵢ` then `f` represents the monomial `∏ᵢ aᵢ ^ nᵢ`. -/
def evalMonomial (f : A →₀ ℕ) : A := ∏ a ∈ f.support, a ^ (f a)

omit noetherian_ring in
@[simp] lemma evalMonomial_zero : evalMonomial (A := A) 0 = 1 := by
  simp [evalMonomial]

/-- Suppose `f` represents the monomial `∏ᵢ aᵢ ^ nᵢ` and each `aᵢ` has degree `dᵢ`, then
the monomial has degree `∑ᵢ nᵢ * dᵢ`. -/
def degreeMonomial (f : A →₀ ℕ) (deg : ⦃a : A⦄ → (ha : a ∈ f.support) → ℕ) : ℕ :=
...
omit noetherian_ring in
lemma evalMonomial_mem_aux {ι : Type*} (s : Finset ι) (deg : s → ℕ)
    (pow : s → ℕ) (f : s → A) (h_deg : ∀ i, f i ∈ 𝒜 (deg i)):
  ...
  · simp only [Finset.attach_insert]
    ...
    · simp only [Subtype.mk.injEq, Subtype.forall, imp_self, implies_true, Finset.prod_image,
        Finset.sum_image]
      apply ih
      rintro ⟨i, hi⟩
...
omit noetherian_ring in
lemma evalMonomial_mem (f : A →₀ ℕ) (deg : ⦃a : A⦄ → (ha : a ∈ f.support) → ℕ)
    (h_deg :  ⦃a : A⦄ → (ha : a ∈ f.support) → a ∈ 𝒜 (deg ha)) :
    evalMonomial f ∈ 𝒜 (degreeMonomial f deg) := by
  convert evalMonomial_mem_aux 𝒜 f.support (fun i ↦ deg i.2)
    (fun i ↦ f i) Subtype.val (fun ⟨i, hi⟩ ↦ h_deg hi)
  exact Finset.prod_attach _ _ |>.symm

variable (S : HomogeneousGeneratingSetOf 𝒜 (HomogeneousIdeal.irrelevant 𝒜).toIdeal)

omit noetherian_ring in
lemma evalMonomial_homogeneous (f : A →₀ ℕ) (hf : f.support ⊆ S.toFinset) :
    IsHomogeneousElem 𝒜 (evalMonomial f) :=
  ⟨degreeMonomial _ _,
    evalMonomial_mem (deg := fun _ h ↦ S.deg (hf h)) (h_deg := fun _ h ↦ S.mem_deg (hf h))⟩

omit noetherian_ring in
lemma top_eq_span_monomial : (⊤ : Submodule (𝒜 0) A) =
    Submodule.span (𝒜 0) {a | ∃ (f : A →₀ ℕ), f.support ⊆ S.toFinset ∧ a = evalMonomial f } := by
  classical
  ...
  refine Algebra.adjoin_induction (hx := hx) (fun x hx ↦ ?_) (fun r ↦ ?_)
    (fun a b _ _ ha hb ↦ ?_) (fun a b _ _ ha hb ↦ ?_)
  ...
  · apply Submodule.span_induction₂ (ha := ha) (hb := hb)
    · rintro _ _ ⟨f, hf, rfl⟩ ⟨g, hg, rfl⟩
      ...
      · exact Finsupp.support_add (g₁ := f) (g₂ := g) |>.trans <| sup_le (α := Finset A) hf hg
      · simp only [evalMonomial, Finsupp.coe_add, Pi.add_apply]
        ...
        rotate_left
        · intro x _ hx2
          simp only [Finsupp.mem_support_iff, Finsupp.coe_add, Pi.add_apply, add_eq_zero,
            not_forall, not_not] at hx2
          rw [pow_add, hx2.1, hx2.2, pow_zero, one_mul]
        · exact Finsupp.support_add (g₁ := f) (g₂ := g) |>.trans <| sup_le (α := Finset A) hf hg
        · intro x _ hx2
          simp only [Finsupp.mem_support_iff, not_not] at hx2
          rw [hx2, pow_zero]
        · intro x _ hx2
          simp only [Finsupp.mem_support_iff, not_not] at hx2
          rw [hx2, pow_zero]
    ...
    · exact fun y _ ↦ (zero_mul y).symm ▸ Submodule.zero_mem _
    · exact fun x _ ↦ (mul_zero x).symm ▸ Submodule.zero_mem _
    · exact fun x₁ x₂ y _ _ _ hx₁ hx₂ ↦ add_mul x₁ x₂ y ▸  Submodule.add_mem _ hx₁ hx₂
    · exact fun x y₁ y₂ _ _ _ hy₁ hy₂ ↦ mul_add x y₁ y₂ ▸ Submodule.add_mem _ hy₁ hy₂
    · intro r x y _ _ h
      change ((r : A) * x) * y ∈ _
...
lemma Finset.single_le_sum' {ι : Type*} {s : Finset ι} {f : s → ℕ} (a : s) :
    f a ≤ ∑ x ∈ s.attach, f x := by
  classical
  ...
  · cases' a with a ha
    ...
    simp only [Subtype.mk.injEq, Subtype.forall, imp_self, implies_true, Finset.sum_image]
    cases' ha with ha ha
    · subst ha
      norm_num
    · specialize ih (f := fun x ↦ f ⟨x.1, by aesop⟩) ⟨a, ha⟩
      exact ih.trans <| by norm_num
      ...
      · have le2 : S.deg (s.2.1 mem1) * s.1 a.1 ≤ k :=
        ...
        · have le4 := HomogeneousGeneratingSetOf.irrelevant.deg_pos S (s.2.1 mem1)
          have le5 : s.1 a.1 ≤ k := by
            ...
            exact this.trans (Nat.div_le_self _ _)
          simpa only [Finset.mem_range, Nat.lt_succ_iff] using le5
      · simp only [Finsupp.mem_support_iff, not_not] at mem1
        rw [mem1]
        aesop⟩
...
instance finite_algebra_over_degree_zero_subring : Algebra.FiniteType (𝒜 0) A :=
  ⟨⟨(HomogeneousGeneratingSetOf.Irrelevant 𝒜).toFinset,
...
/-- A finite homogeneous generating set of a submodule `p` is a set of non-zero
homogeneous elements that spans `p`. -/
...
noncomputable def deg {a : M} (h : a ∈ S) : ℕ := S.homogeneous h |>.choose

omit finite_module noetherian_ring [DirectSum.Decomposition ℳ] in
lemma mem_deg {a : M} (h : a ∈ S) : a ∈ ℳ (S.deg h) := S.homogeneous h |>.choose_spec

variable (A ℳ) in
/-- An arbitrary chosen finite generating set for the top submodule. -/
noncomputable def Top : HomogeneousGeneratingSetOf ℳ (⊤ : Submodule A M) :=
...
lemma generatingSet_is_finite (k : ℕ) :
    {x : ℳ k | ∃ (ω : M) (_ : ω ∈ TM.toFinset) (p : A →₀ ℕ) (hp1 : p.support ⊆ T.toFinset),
      degreeMonomial p (fun _ h ↦ T.deg (hp1 h)) ≤ k ∧ (x : M) = evalMonomial p • ω }.Finite := by
  let S := {x : ℳ k | ∃ (ω : M) (_ : ω ∈ TM.toFinset) (p : A →₀ ℕ) (hp1 : p.support ⊆ T.toFinset),
...
variable {𝒜 ℳ} in
lemma kth_degree_eq_span (k : ℕ) : (⊤ : Submodule (𝒜 0) (ℳ k)) =
    Submodule.span (𝒜 0)
      {x : ℳ k | ∃ (ω : M) (_ : ω ∈ TM.toFinset) (p : A →₀ ℕ) (hp1 : p.support ⊆ T.toFinset),
        degreeMonomial p (fun _ ha ↦ T.deg (hp1 ha)) ≤ k ∧ (x : M) = evalMonomial p • ω } := by
  refine le_antisymm (fun ⟨x, hx⟩ _ ↦ ?_) le_top
  ...
  replace eq0 := calc
      ...
      _ = ∑ i ∈ f.support, ∑ j ∈ (r (f i)).support, (r (f i) j : A) • (j : A) • (i : M) :=
            Finset.sum_congr rfl fun x _ ↦ Finset.sum_congr rfl fun y _ ↦ by
              ...
              rw [mul_smul]
      _ = ∑ i ∈ f.support, ∑ j ∈ (r (f i)).support.attach, (r (f i) j : A) • (j : A) • (i : M) :=
  ...
  replace eq0 := calc
    ...
    _ = ∑ i ∈ f.support.attach, ∑ j ∈ (r (f i)).support.attach,
          ...
          then GradedRing.proj 𝒜 (k - TM.deg (f_support_le i.2))
            ((r (f i) j : A) * (evalMonomial (p _ _ j.2) : A)) • (i : M) else 0 :=
          Finset.sum_congr rfl fun i _ ↦ Finset.sum_congr rfl fun j _ ↦ by
            rw [GradedModule.proj_smul_mem_right 𝒜 ℳ _ _ (TM.mem_deg (f_support_le i.2))]
    _ = ∑ i ∈ f.support.attach.filter fun i : f.support ↦ TM.deg (f_support_le i.2) ≤ k,
          ∑ j ∈ (r (f i)).support.attach, GradedRing.proj 𝒜 (k - TM.deg (f_support_le i.2))
    ...
    _ = ∑ i ∈ f.support.attach.filter fun i : f.support ↦ TM.deg (f_support_le i.2) ≤ k,
          (∑ j ∈ (r (f i)).support.attach, if degreeMonomial (p _ _ j.2)
            (fun _ h ↦ T.deg (hp1 _ _ j.2 h)) ≤ k - TM.deg (f_support_le i.2)
              then GradedRing.proj 𝒜 ((k - TM.deg (f_support_le i.2)) -
                degreeMonomial (p _ _ j.2) fun a ha ↦ T.deg (hp1 _ _ j.2 ha))
                  (r (f i) j : A) * (evalMonomial (p _ _ j.2) : A) else 0) • (i : M) := by
          refine Finset.sum_congr rfl fun ⟨i, hi1⟩ _ ↦ ?_
          congr 1
          refine Finset.sum_congr rfl fun ⟨j, hj⟩ _ ↦ ?_
          rw [GradedRing.proj_apply, DirectSum.coe_decompose_mul_of_right_mem 𝒜]
          pick_goal 2
          · apply evalMonomial_mem 𝒜 (p _ _ hj) (fun _ h ↦ T.deg (hp1 _ _ hj h))
            rintro a ha
            exact T.mem_deg (hp1 _ _ hj ha)
          rfl
    _ = ∑ i ∈ f.support.attach.filter fun i : f.support ↦ TM.deg (f_support_le i.2) ≤ k,
    ...
    _ = ∑ i ∈ f.support.attach.filter fun i : f.support ↦ TM.deg (f_support_le i.2) ≤ k,
          ...
          (if degreeMonomial (p _ _ j.2) (fun a ha ↦ T.deg (hp1 _ _ j.2 ha)) =
            k - TM.deg (f_support_le i.2) then (r (f i) j : A) else 0) •
              (evalMonomial (p _ _ j.2) : A) • (i : M) :=
    ...
    _ = ∑ i ∈ f.support.attach.filter fun i : f.support ↦ TM.deg (f_support_le i.2) ≤ k,
          ...
          refine Finset.sum_congr rfl fun ⟨h, hj1⟩ _ ↦ ?_
        ...
        degreeMonomial (p _ _ j.2) (fun a ha ↦ T.deg (hp1 _ _ j.2 ha)) =
          k - TM.deg (f_support_le i.1.2)).attach,
            r (f i) j • (⟨(evalMonomial (p _ _ j.1.2) : A) • (i : M), by
              ...
              have mem := j.2
              simp only [Finset.mem_filter, Finset.mem_attach, true_and] at mem
              rw [mem, vadd_eq_add, Nat.sub_add_cancel]
              simpa [-Finset.coe_mem] using i.2⟩ : ℳ k) := by
    ext
    refine eq0.trans ?_
    rw [AddSubgroup.val_finset_sum]
    refine Finset.sum_congr rfl fun ⟨j, hj1⟩ _ ↦ ?_
    rw [AddSubgroup.val_finset_sum]
    rfl
  rw [eq0]
  refine Submodule.sum_mem _ fun ⟨⟨i, hi1⟩, hi2⟩ _ ↦ Submodule.sum_mem _ fun ⟨⟨j, hj1⟩, hj2⟩ _ ↦ ?_
  simp only [Finset.mem_filter, Finset.mem_attach, true_and] at hj2
  exact Submodule.smul_mem _ _ <| Submodule.subset_span
    ⟨i, f_support_le hi1, p _ _ hj1, hp1 _ _ hj1, hj2 ▸ Nat.sub_le _ _, rfl⟩
...
instance finite_module_over_degree_zero_subring (k : ℕ) : Module.Finite (𝒜 0) (ℳ k) :=
  ⟨Set.Finite.toFinset (generatingSet_is_finite
    ...
    by simpa only [Set.Finite.coe_toFinset] using
      (kth_degree_eq_span (GradedRing.HomogeneousGeneratingSetOf.Irrelevant 𝒜)
